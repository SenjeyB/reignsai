function scrResolveResponse(){
	...
	if (status < 0) {
	    ...
	    if (is_session_init) {
	        ...
	        if (global.api_session_init_req == req_key) {
	            global.api_session_init_pending = false;
	            global.api_session_init_req = "";
	        }
	        scrApiSetError("No connection to server. Check that the server is running and reachable.");
	        exit;
	    }
	    ...
	    if (global.http_retry_count < global.http_retry_max) {
	        ...
	        var _target = ds_map_find_value(global.http_save_targets, req_key);
	        ds_map_delete(global.http_save_targets, req_key);
	        global._retry_filename = _target;
	        if (instance_exists(objCardRequester)) {
	            with (objCardRequester) {
	                alarm[0] = 10;
	            }
	        } else {
	            alarm[0] = 10;
	...
	if (http_status != 200) {
	    ...
	    scrApiSetError("Server error (HTTP " + string(http_status) + ").");
	    exit;
	}
	...
	if (is_session_init) {
	    ...
	    if (is_struct(init_parsed) && variable_struct_exists(init_parsed, "success") && init_parsed.success) {
	        ...
	        if (is_struct(init_data) && variable_struct_exists(init_data, "session_id")) {
	            ...
	            scrApiSetError("Server returned an invalid session response.");
	        }
	    } else {
	        show_debug_message("Session init failed: " + json_text);
	        scrApiSetError("Server failed to initialize a session.");
	    }
	...
	if (is_struct(parsed) && variable_struct_exists(parsed, "success")) {
	    if (!parsed.success) {
	        ...
	        show_debug_message("Server returned error: " + error_msg);
	        ...
	        scrApiSetError("Server: " + string(error_msg));
	        exit;
	    }
	}

	var _cards = scrParseCardsFromJsonText(json_text);
	if (array_length(_cards) <= 0) {
	    show_debug_message("Server returned no cards");
	    ds_map_delete(global.http_save_targets, req_key);
	    if (global.waiting_req == req_key) {
	        global.waiting_http = false;
	        global.waiting_req = "";
	    }
	    scrApiSetError("Server returned no cards.");
	    exit;
	}
